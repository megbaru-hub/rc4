...
int decrypt_file(const char *filepath, const unsigned char *key, size_t key_len) {
    ...
    if (file_size > 0) {
        prga(S, buffer, plaintext, file_size);
    }
...
int main(int argc, char *argv[]) {
    ...
    return decrypt_file(filepath, (const unsigned char *)key, strlen(key));
}
